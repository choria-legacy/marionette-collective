...
4. [Change the port and protocol][activemq_transports] on the stomp transport connector to `stomp+nio+ssl://0.0.0.0:61614?needClientAuth=true`, since we'll be using CA-verified TLS. (If you are running ActiveMQ 5.5.x or 5.6.x, set it to `stomp+ssl://0.0.0.0:61614?needClientAuth=true` instead; the stomp+nio+ssl protocol had a bug on 5.6 and didn't exist in 5.5.)
